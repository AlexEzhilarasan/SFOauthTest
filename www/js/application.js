...
var templates = {
    ...
    uploadForm:"views/uploadForm.html",
    loaded: 0,
    requested: 0,
};
...
function saveFormData( event ) {
    ...
    try {
        if ( currentLead == undefined ) {
            sfw.client.create("Alex_DB_Object__c", data, saveDataSuccess, saveDataError );
        } else {
            sfw.client.update("Alex_DB_Object__c", currentLead.Id, data, saveDataSuccess, saveDataError );
        }
    } 
    catch(e){
        console.log(e);
...
function queryRecords() {
    ...
    var query = "SELECT Email__c,Name__c,Id,Latitude__c,Longitude__c "+
    "FROM Alex_DB_Object__c " + 
    "ORDER BY Name__c"
